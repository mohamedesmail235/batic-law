baticApp.doctypes['sales-invoice'] = {
        ...
        if (form && form.items) {
            ...
            for (let key in items) {
                if (items.hasOwnProperty(key)) {
                    ...
                    if ((item.qty) && (item.rate && item.rate >= 0)) {
            ...
            if (form.total_taxes_and_charges) {
    ...
    data: function ($rootScope) {
        let data = {
            ...
            "set_warehouse": stringVal($rootScope.formView.form.data.set_warehouse),
            "due_date": ($rootScope.formView.form.data.due_date) ? moment($rootScope.formView.form.data.due_date).format('YYYY-MM-DD') : moment().format('YYYY-MM-DD'),
            "is_return": ($rootScope.formView.form.data.is_return == 1) ? 1 : 0,
            ...
            "update_stock": ($rootScope.formView.form.data.update_stock == 1) ? 1 : 0,
        ...
        if ($rootScope.formView.form.data.return_against && $rootScope.formView.form.data.return_against.length)
            data['return_against'] = $rootScope.formView.form.data.return_against;
        return data;
    },
    events: function (form_data, $rootScope, $scope, $http, $timeout) {
        ...
        $(document).on('change', '#field-table-items [data-fieldname="item_code"]', function () {
            ...
            if (!$(this).val())
                return
            frappe.db.get_value('Item', {'name': $(this).val()}, ['item_name', 'stock_uom', 'standard_rate', 'standard_buying_rate']).then(function (response) {
                $timeout(() => {
