baticApp.doctypes['consultations'] = {
    ...
    events: function (form_data, $rootScope, $scope, $http, $timeout) {
        $(document).on('field-link-blur', '[data-fieldname="quotation"]', function () {
            if (form_data.quotation && form_data.quotation.length) {
                $http.post(frappe_api_url('frappe.client.get'), {
                    doctype: 'Quotation',
                    name: form_data.quotation
                }).then(function ({data: {message}}) {
                    $timeout(function () {
                        form_data.customer = message.customer_name;
                    }, 10);
                });
            }
        });
