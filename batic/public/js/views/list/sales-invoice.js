baticApp.list_view['sales-invoice'] = {
    columns: [
        {
            name: 'name',
            label: baticApp.app.__lang('Name'),
            template: `<a class="docname-link" ui-sref="app.form({parent:current_route_params.parent,doctype:'sales-invoice',type:'view',docname:row.name})">{{row.name}}</a>`
        },
        {
            name: 'docstatus',
    ...
    status: {
        ...
        'Partly Paid': {
            label: baticApp.app.__lang('Partly Paid'),
            color: 'green'
        },
